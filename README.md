...
| 随时导出 BCS | 从字幕窗口或右上角更多菜单导出当前项目全部图片、气泡坐标和样式，方便备份或再次导入 |
| AI 脚本指南 | 自动附加当前项目的真实图片顺序、图片尺寸和完整脚本模板 |
| 移动端完整编辑 | Android 与 iOS 竖屏使用全屏画布、底部步骤栏和上拉属性面板；横屏使用图片、画布、属性三栏布局 |
...
3. **检查、微调并导出**
   ...
   在画布中修改气泡位置、大小、字体和颜色。右上角“批量导出”可得到多张 PNG 成图；“更多操作 → 导出完整 BCS 字幕”可随时保存当前项目的可再次导入脚本。
